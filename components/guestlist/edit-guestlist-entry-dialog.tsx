...
import { updateGuestlistEntry } from "@/lib/actions"
...
interface EditGuestlistEntryDialogProps {
  open: boolean
  onOpenChange: (open: boolean) => void
  entry: GuestlistEntry
}

export function EditGuestlistEntryDialog({ open, onOpenChange, entry }: EditGuestlistEntryDialogProps) {
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    try {
      await updateGuestlistEntry(entry.id, values)
      toast({
        title: "Entry updated",
        description: `${values.name}'s guestlist entry has been updated.`,
      })
      onOpenChange(false)
    } catch (error) {
      toast({
        title: "Error",
        description: "Failed to update guestlist entry. Please try again.",
        variant: "destructive",
      })
    } finally {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          <DialogTitle>Edit Guestlist Entry</DialogTitle>
          <DialogDescription>Update {entry.name}'s guestlist information.</DialogDescription>
        </DialogHeader>
